...
router.post("/upload", authMiddleware, async (req, res) => {
    try {
        ...
        return res.json({ success: true, url: publicUrl, name: fileName, size: buffer.byteLength, type: fileType });
    } catch (e) {
...
router.get("/:id", authMiddleware, async (req, res) => {
    try {
        ...
        if (req.user.app_role !== "admin") query = query.eq("created_by", req.user.id);
...
router.post("/", authMiddleware, async (req, res) => {
    try {
        const { Summary, Description, Type, Department, Category, Site, attachments } = req.body;

        if (!Summary || !Description) {
            return res.status(400).json({ success: false, message: "Summary and Description are required" });
        }
